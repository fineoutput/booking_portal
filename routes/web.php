...
Route::group(['prefix' => '/'], function () {
    ...
    Route::get('final_booking/{id}', [HomeController::class, 'final_booking'])->name('final_booking');
    
    Route::post('/hotel/calculate-price/{id}', [HomeController::class, 'calculatePrice'])
    ->name('calculate_hotel_price');

    // routes/web.php
Route::get('/add-tourists/{booking}/{status}', [HomeController::class, 'touristscreate'])->name('tourists.create');
Route::post('/add-tourists/{booking}/{status}', [HomeController::class, 'store'])->name('tourists.store');
    ...
    Route::post('/save-tourist-details-guide', [HomeController::class, 'saveGuideTouristDetails'])->name('saveGuideTouristDetails');
    ...
    Route::get('/download-pdf-with-logo/{user_id}/{booking_id}/{pdf_name}', [PDFController::class, 'downloadWithLogo'])->name('pdf.download');

    Route::get('/invoice/{id}', [HomeController::class, 'invoice'])->name('invoice.download');
    


    Route::post('/upgrade-request', [HomeController::class, 'upgrade_request'])->name('upgrade_request');
    Route::post('/upgrade-request-hotel', [HomeController::class, 'upgrade_requesthotel'])->name('upgrade_requesthotel');
  ...
  Route::post('/agent/wallet/add', [HomeController::class, 'addWalletWeb'])->name('wallet.store');
Route::post('/agent/wallet/razorpay/callback', [HomeController::class, 'walletRazorpayCallback'])->name('wallet.razorpay.callback');
